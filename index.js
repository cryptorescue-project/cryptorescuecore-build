 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`cryptorescuecore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`cryptorescuecore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'cryptorescuecore-' + name : 'cryptorescuecore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/cryptorescuecore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external cryptorescuecore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:cryptorescuecore-lib -o cryptorescuecore-lib.js';
    }
  ...
  gulp.task('release:push', function(cb) {
    git.push('cryptorescue-project', 'master', {
      args: ''
    }, cb);
  });
  ...
  gulp.task('release:push-tag', function(cb) {
    ...
    git.tag(name, 'Release ' + name, function() {
      git.push('cryptorescue-project', name, cb);
    });
  });
  ...
  var release = function(importance, cb) {
    ...
    return runsequence(
      ...
      // Run git push cryptorrescue-project $VERSION
      'release:push-tag',
      // Run npm publish
      'release:publish',
